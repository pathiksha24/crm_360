...
$aColumns = array_merge($aColumns, [
    ...
    '(SELECT GROUP_CONCAT(description SEPARATOR ", ") FROM ' . db_prefix() . 'notes WHERE rel_id = ' . db_prefix() . 'leads.id AND rel_type = "lead") as notes',
...
$service_id      = $this->ci->input->post('service_id');
...
if ($status && !in_array('',$status)
    ...
    array_push($where, 'AND '.db_prefix().'leads.status IN (' . implode(',', $status) . ')');
...
$allowed_for_anju = [17, 58, 14, 59, 55, 216, 214, 72, 20, 34, 163, 54, 56,178]; // keep 77 OUT if Quality should not see
...
// call center staffs, poluolny, call center 1  and anju can see the leads which is assigned to anju
	...
	$row[] = ($aRow['dateassigned'] == '0000-00-00 00:00:00' || !is_date($aRow['dateassigned']) ? '' : '<span style="display:none;">'.strtotime($aRow['dateassigned']).'</span>'. _dt($aRow['dateassigned']));
