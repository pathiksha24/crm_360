...
class Si_lead_filters extends AdminController 
{
	...
	private function get_where_report_period($field = 'date',$months_report='this_month')
	{
		...
		if ($months_report != '') {
			if (is_numeric($months_report)) {
                ...
                $from_date = to_sql_date($this->input->post('report_from'), true);
                $to_date   = to_sql_date($this->input->post('report_to'), true);
	...
	{
        //$this->db->query("ALTER TABLE `tblleads` MODIFY COLUMN `dateassigned` DATETIME");
//        $leads = $this->db->select('id')->get('tblleads')->result();
//
//        foreach ($leads as $lead) {
//            $this->db->where('leadid', $lead->id);
//            $this->db->where('description', 'not_lead_activity_assigned_to');
//            $this->db->order_by('date', 'asc');
//            $lead_activity_log = $this->db->get(db_prefix() . 'lead_activity_log')->row();
//
//            if ($lead_activity_log) {
//                $this->db->where('id', $lead->id);
//                $this->db->update('tblleads', ['dateassigned' => $lead_activity_log->date]);
//            }
//        }
//        die();
		$overview = [];
        ...
        if ($this->input->post('has_notes')) {
            $selected_has_notes = $this->input->post('has_notes');
        } else {
            $selected_has_notes = '';
        }

        if ($this->input->post('team_leader')) {
            $selected_team_leader = $this->input->post('team_leader');
        } else {
        ...
        $data['selected_has_notes']  = $selected_has_notes;
        $data['selected_team_leader']  = $selected_team_leader;
	...
	function table()
	{
		...
		$filter_by_assigned = false;
		if ($data['report_months'] != ''){
//            if ($date_by != 'dateassigned') {
//                $date_by = 'DATE('.$date_by.')';
//            }
			$report_months = $data['report_months'];
			$data['custom_date_select'] = $this->get_where_report_period($date_by,$report_months);
		}
		$data['perfex_version'] = (int)$this->app->get_current_db_version();
		$data['filter_by_assigned'] = $filter_by_assigned;
    ...
    public function fix_old_leads_v2()
    {
        $this->load->model('leads_model');
        $leads = $this->leads_model->get('1207386');
        echo '<pre>';
        print_r($leads);
        echo '</pre>';
        die();
