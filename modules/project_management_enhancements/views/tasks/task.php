...
} ?>" id="_task_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" <?php if ($this->input->get('opened_from_lead_id'))
{
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
						...
						{
							echo '<div class="alert alert-success text-center no-margin">'._l(
									'task_is_billed',
									'<a href="'.admin_url('invoices/list_invoices/'.$task->invoice_id).'" target="_blank" class="alert-link">'.e(format_invoice_number($task->invoice_id))
								).'</a></div><br />';
						}
						?>
						<?php if (isset($task)) { ?>
                                ...
                                <div class="content-menu hide">
                                    <ul>
										<?php if (staff_can('create', 'tasks')) { ?>
											...
											{
												$copy_template .= "<div class='checkbox checkbox-primary'><input type='checkbox' name='copy_task_assignees' id='copy_task_assignees' checked><label for='copy_task_assignees'>"._l(
														'task_single_assignees'
													).'</label></div>';
											}
											if (total_rows(db_prefix().'task_followers', ['taskid' => $task->id]) > 0)
											{
												$copy_template .= "<div class='checkbox checkbox-primary'><input type='checkbox' name='copy_task_followers' id='copy_task_followers' checked><label for='copy_task_followers'>"._l(
														'task_single_followers'
													).'</label></div>';
											}
											if (total_rows(db_prefix().'task_checklist_items', ['taskid' => $task->id]) > 0)
											{
												$copy_template .= "<div class='checkbox checkbox-primary'><input type='checkbox' name='copy_task_checklist_items' id='copy_task_checklist_items' checked><label for='copy_task_checklist_items'>"._l(
														'task_checklist_items'
													).'</label></div>';
											}
											if (total_rows(db_prefix().'files', ['rel_id' => $task->id, 'rel_type' => 'task']) > 0)
											{
												$copy_template .= "<div class='checkbox checkbox-primary'><input type='checkbox' name='copy_task_attachments' id='copy_task_attachments'><label for='copy_task_attachments'>"._l(
											...
											{
												$copy_template .= "<div class='radio radio-primary'><input type='radio' value='".$copy_status['id']."' name='copy_task_status' id='copy_task_status_".$copy_status['id']."'".($copy_status['id'] == hooks(
													)->apply_filters('copy_task_default_status', 1) ? ' checked' : '')."><label for='copy_task_status_".$copy_status['id']."'>".$copy_status['name'].'</label></div>';
											}
                        ...
                        <div class="task-visible-to-customer tw-pt-2 checkbox checkbox-inline checkbox-primary<?php if ((isset($task) && $task->rel_type != 'project') || ! isset($task) || (isset($task) && $task->rel_type == 'project' && total_rows(
									db_prefix().'project_settings',
									['project_id' => $task->rel_id, 'name' => 'view_tasks', 'value' => 0]
								) > 0))
						{
							echo ' hide';
						} ?>">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="task_type" class="selectpicker" id="task_type" data-width="100%"
                                        ...
                                        <option <?php echo(! isset($task) || empty($task->task_type) ? 'selected' : '') ?> disabled hidden><?php echo _l('dropdown_non_selected_tex'); ?></option>
										<?php foreach ($task_types as $task_type) { ?>
                                            <option style="background-color: <?php echo $task_type['label_color'] ?>;color: <?php echo $task_type['text_color'] ?>"
                                                    value="<?php echo e($task_type['id']); ?>" <?php if (isset($task) && $task->task_type == $task_type['id'])
											{
												echo ' selected';
											} ?>><?php echo e($task_type['name']); ?></option>
										<?php } ?>
                                    </select>
                                </div>
                            </div>
						...
						echo render_textarea(
							...
							[
								...
								! is_mobile() ? 'onclick' : 'onfocus' => (! isset($task) || isset($task) && $task->description == '' ? 'init_task_description_editor(\'.tinymce-task\', {height:200, auto_focus: true});' : ''),
							],
							[],
							'no-mbot',
							'tinymce-task'
    ...
    <script>
        ...
        function init_task_description_editor(selector, settings) {
            console.log('here')
            ...
            var _settings = {
                ...
                paste_preprocess: (plugin, args) => {
                    if (args.content.includes('loom.com/share/')) {
                        args.preventDefault();
