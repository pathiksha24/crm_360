...
class Tasks extends AdminController {
	public function __construct()
	{
		...
		$this->load->model('tasks_enhance_model');
	}
	...
	public function detailed_overview()
	{
		...
		for ($m = 1; $m <= 12; $m++)
		{
			...
			if (is_numeric($staff_id))
			{
				$sqlTasksSelect .= ',(SELECT COUNT(id) FROM '.db_prefix().'task_checklist_items WHERE taskid='.db_prefix(
	...
	public function get_task_data($taskid, $return = FALSE)
	{
		...
		$task = $this->tasks_enhance_model->get($taskid, $tasks_where);
		...
		$project_owners = $this->projects_enhance_model->get_project_owners($task->rel_id);
		$this->tasks_enhance_model->update_project_owners($taskid, $project_owners);
		...
		hooks()->do_action('user_read_recent_change', $task->id, get_staff_user_id());
	...
	/**
	 * Reply task comment / ajax
	 * @return void
	 */
	public function reply_task_comment()
	{
		$data = $this->input->post();
		hooks()->add_filter('html_purify_content', function ($will_purify) {
			return FALSE;
		...
		$comment_id = $data['parent_id'] ?? FALSE;
		if ( ! empty($data['content']) && $comment_id)
		{
			$comment_id = $this->tasks_enhance_model->add_task_comment($data);
		}

		if ($comment_id)
		{
			$task = $this->tasks_enhance_model->get($data['taskid']);
			hooks()->do_action('task_recent_change', $task);
		}
		echo json_encode([
			'success' => $comment_id ? TRUE : FALSE,
			'taskHtml' => $this->get_task_data($data['taskid'], TRUE),
			'message' => $comment_id ? _l('replied_task_comment') : _l('failed_to_reply_comment'),
		]);
	}
	...
	public function mark_as($status, $id)
	{
		...
		)
		{
			...
			if ($success)
			{
				...
				$task = $this->tasks_enhance_model->get($id);
				hooks()->do_action('task_recent_change', $task);
			}
	...
	public function add_task_comment_image()
	{
		$images = $_FILES ?? NULL;
		$task_id = $_REQUEST['taskid'] ?? NULL;
		if (empty($images) || empty($task_id))
		{
			echo json_encode([
				'success' => FALSE,
				'url' => '',
		...
		{
			$uploaded_image = handle_task_comment_image_upload($_FILES['image'], $task_id);
			echo json_encode([
				'success' => TRUE,
				'url' => $uploaded_image,
			]);
